...
import vortex.data.notes.NoteDirection;
...
class SustainTrail extends FlxSprite
{
  ...
  public function updateClipping(songTime:Float = 0):Void
  {
    ...
    // Top right
    uvtData[1 * 2] = uvtData[0 * 2] + 0.5; // 12.5%/37.5%/62.5%/87.5% of the way through the image (1/8th past the top left)
  ...
  override public function draw():Void
  {
    ...
    for (camera in cameras)
    {
      ...
      camera.drawTriangles(processedGraphic, vertices, indices, uvtData, null, _point, blend, true, antialiasing, null, shader);
    }
